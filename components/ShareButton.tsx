...
import { Copy, Download, Link, MessageCircle, Send, Share2, X } from "lucide-react";
...
export function ShareButton({ target, result }: ShareButtonProps) {
  ...
  const shareUrl = typeof window === "undefined" ? "https://startup-82-0.local/result" : window.location.href;
  ...
  async function downloadImage() {
    ...
    link.href = dataUrl;
    link.download = "startup-82-0-result.png";
    link.click();
  }
  ...
  function openModal() {
    trackGameEvent("share_clicked", { outcome: result.tier, score: result.score });
    setOpen(true);
  }
          ...
          <div className="max-h-[92vh] w-full max-w-3xl overflow-auto rounded-[20px] border border-[var(--line)] bg-[var(--bg-strong)] p-5 shadow-2xl">
            ...
            <div className="mt-5 rounded-[var(--radius)] border border-[var(--line)] bg-[var(--surface)] p-5">
              <div className="flex items-center justify-between border-b border-[var(--line)] pb-4">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.24em] text-[var(--muted)]">Outcome</p>
                  <p className="display-font mt-2 text-4xl">{result.valuation}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-black uppercase tracking-[0.24em] text-[var(--muted)]">Score</p>
                  <p className="mt-2 text-4xl font-black text-[var(--gold)]">{result.score}</p>
                </div>
              </div>
              <div className="mt-4 grid gap-2">
                {result.team.filter((slot) => slot.person).map(({ role, person }) => (
                  <div key={role} className="grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-[var(--radius)] bg-[var(--bg)] p-3">
              ...
              <p className="mt-5 text-lg font-black">I built a {result.valuation} startup. Think you can beat it?</p>
            </div>
