...
export const roles: Role[] = ["ceo", "cto", "product", "growth"];
...
export function roleFitScore(person: Person, role: Role) {
  ...
  const roleBonus =
    person.primaryRole === role ? 10 : person.secondaryRole === role ? 5 : 0;
...
export function groupCanFillOpenRole(
  groupId: string,
  team: Partial<Record<Role, string>>,
  pickedIds: string[] = [],
) {
  ...
  return group.peopleIds.some((personId) => {
    ...
    return (
      ...
      getCompatibleRoles(person, team).length > 0
    );
  });
}
...
function getNextGroupId(
  ...
  const compatibleGroups = sourceGroups.filter((group) =>
    groupCanFillOpenRole(group.id, team, pickedIds),
  );
  const unused = compatibleGroups.filter(
...
export function createGame(
  ...
  return {
    id: createId(),
    companyName: companyName.trim() || "Pied P",
    round: 1,
    maxRounds,
    currentGroupId,
    team: {},
...
export function getCurrentGroup(state: GameState) {
  return (
    sourceGroups.find((group) => group.id === state.currentGroupId) ??
    sourceGroups[0]
  );
}
...
export function getRoundChoices(state: GameState) {
  ...
  const groupPeople = group.peopleIds
    .map((id) => peopleById.get(id))
    .filter(Boolean) as Person[];
  const roundPickIds = new Set(
    state.picks.map((pick) => pick.selectedPersonId),
  );
  const available = groupPeople.filter(
    (person) => !roundPickIds.has(person.id),
  );
  return shuffle(available);
}

export function assignPersonToTeam(
  team: Partial<Record<Role, string>>,
  person: Person,
  role: Role,
) {
...
export function placePerson(
  ...
  const nextPickedIds = [...state.picks, pick].map(
    (item) => item.selectedPersonId,
  );
  const nextGroupId = completed
    ? state.currentGroupId
    : getNextGroupId(state.usedGroupIds, nextTeam, nextPickedIds);
  ...
  return {
    state: {
      ...
      usedGroupIds: completed
        ? state.usedGroupIds
        : [...state.usedGroupIds, nextGroupId],
    },
    assignment,
    completed,
  };
...
export function spinGroup(state: GameState) {
  ...
  const nextGroupId = getNextGroupId(
    [...state.usedGroupIds, state.currentGroupId],
    state.team,
    pickedIds,
  );
...
export function getPickedPeople(picks: GamePick[]) {
  return picks
    .map((pick) => peopleById.get(pick.selectedPersonId))
    .filter(Boolean) as Person[];
}
